...
class Distilling_Mask(Distilling_Single):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 distill=None,):
        super(Distilling_Mask, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def forward_train(self,
        ...
        for layer in range(layers):
            ...
            # breakpoint()            
            n, c, h, w = tea_cls_score_sigmoid.shape
            num_classes = 80
            target = F.one_hot(self.target_list[layer], num_classes=num_classes + 1)[:, :num_classes]
            target = target.reshape(n, h, w, -1).permute(0,3,1,2)
            mask = torch.max(target * tea_cls_score_sigmoid, dim=1).values
